...
/**
 * 2次ウェーブレット変換のモデルクラス
 * このクラスは、2次元ウェーブレット変換のデータと、その変換に関連する操作を管理する。
...
public class Wavelet2dModel extends WaveletModel {
    ...
    /**
     * Wavelet2dModelのコンストラクタ。
     * インスタンス生成時にサンプル係数データをロードして初期化する。
     */
    ...
    /**
     * アクションイベントを処理する。
     * メニュー項目からのコマンドに応じて、異なる画像データの設定や係数操作を実行する。
    ...
    public void actionPerformed(ActionEvent anActionEvent) {
        ...
        if (commandString == "smalltalk balloon") {
            doSmalltalkBalloon();
            return;
        } 
        if (commandString == "earth") {
            doEarth();
            return;
        } 
        if (commandString == "all coefficients") {
            doAllCoefficients();
            return;
        } 
        if (commandString == "clear coefficients") {
            doClearCoefficients();
            return;
        } 
    }
            ...
            if (sourceData != null) { // データが存在する場合のみ処理
                ...
                for (byte rowOffset = -2; rowOffset <= 2; rowOffset++) {
                    for (byte colOffset = -2; colOffset <= 2; colOffset++) {
                        ...
                        // 座標を配列の範囲内に制限
                        adjustedX = Math.min(Math.max(0, adjustedX), coefficientWidth - 1);
                        adjustedY = Math.min(Math.max(0, adjustedY), coefficientHeight - 1);
                        if (isAltDown) {
    ...
    /**
     * インタラクティブなウェーブレット係数を使用して画像を再構成する。
     * 再構成された係数から画像を生成し、関連する表示モデルを更新する。
     */
    ...
    /**
     * サンプルの2次元係数データ（四角い枠と対角線）を生成する。
     * このデータは初期表示やデモンストレーションに使用される。
     * @return サンプルの2次元係数行列
     */
    ...
    /**
     * 3次元の係数行列（例：LuminanceまたはRGB各チャネル）からBufferedImageを生成する。
     * 輝度画像またはRGB画像を生成する。
        ...
        if (valueMatrixArray[1] == null || valueMatrixArray[2] == null || valueMatrixArray[3] == null) {
            for (byte rowIndex = 0; rowIndex < imageHeight; rowIndex++) {
                for (byte colIndex = 0; colIndex < imageWidth; colIndex++) {
                    ...
                    Integer grayValue = (Integer)Math.round(absoluteValue / maxValue * 255.0D); // 0-255に正規化
            ...
            for (byte rowIndex = 0; rowIndex < imageHeight; rowIndex++) {
                for (byte colIndex = 0; colIndex < imageWidth; colIndex++) {
                    ...
                    Integer redValue = (Integer)Math.round(redCoefficient / maxValue * 255.0D);
                    double greenCoefficient = Math.abs(valueMatrixArray[2][colIndex][rowIndex]); // Gチャネル
                    Integer greenValue = (Integer)Math.round(greenCoefficient / maxValue * 255.0D);
                    double blueCoefficient = Math.abs(valueMatrixArray[3][colIndex][rowIndex]); // Bチャネル
                    Integer blueValue = (Integer)Math.round(blueCoefficient / maxValue * 255.0D);
    ...
    /**
     * 2次元の係数行列からBufferedImageを生成する。
     * 主に個々のウェーブレット係数やスケーリング係数を視覚化するために使用される。
    ...
    public static BufferedImage generateImage(double[][] valueMatrix, Point scaleFactor, Integer rgbFlag) {
        ...
        for (rowIndex = 0; rowIndex < matrixHeight; rowIndex++) {
            for (byte colIndex = 0; colIndex < matrixWidth; colIndex++) {
                double currentValue = Math.abs(valueMatrix[colIndex][rowIndex]);
                Integer normalizedValue = (Integer)Math.round(currentValue / maxAbsoluteValue * 255.0D); // 0-255に正規化
     ...
     * 1つの大きなBufferedImageを生成する。
     * このメソッドは、ウェーブレット変換の各成分を視覚的に並べて表示するために使用される。
     *
     * 生成される画像レイアウトは以下の通り:
     * +--------------------------------+--------------------------------------+
    ...
    /**
     * BufferedImageを輝度（Luminance）とRGB（赤、緑、青）の各成分を表す
     * 2次元行列の配列に変換する。
     * @param anImage 変換するBufferedImage
     * @return 輝度およびRGB成分のdouble型2次元行列の配列。
     * 配列のインデックス0: 輝度, 1: R, 2: G, 3: B
     */
    ...
    /**
     * 全てのチャネルのスケーリング係数の絶対値の最大値を計算して返す。
     * 計算済みの場合はキャッシュされた値を返す。
    ...
    /**
     * 全てのチャネルのソース係数（元の画像データ）の絶対値の最大値を計算して返す。
     * 計算済みの場合はキャッシュされた値を返す。
    ...
    /**
     * 全てのチャネルの再構成された係数の絶対値の最大値を計算して返す。
     * 計算済みの場合はキャッシュされた値を返す。
    ...
    /**
     * 全てのチャネルのウェーブレット係数（水平、垂直、対角）の絶対値の最大値を計算して返す。
     * 計算済みの場合はキャッシュされた値を返す。
    ...
    /**
     * マウスクリックイベントを処理する。
     * クリックされた座標に基づいてウェーブレット係数を計算し、画像を更新する。
    ...
    /**
     * マウスドラッグイベントを処理する。
     * ドラッグ中の座標に基づいてウェーブレット係数を計算し、画像を更新する。
    ...
    /**
     * メインのGUIウィンドウを開き、ウェーブレット変換の結果を表示する。
     * GridBagLayoutを使用して4つの表示パネルを配置する。
     */
    public void open() {
    ...
    /**
     * 3次元の係数行列配列（輝度およびRGB各チャネル）をソースデータとして設定し、
     * ウェーブレット変換を実行して、結果の係数と表示モデルを更新する。
    ...
    /**
     * 右クリックイベント時にポップアップメニューを表示する。
     * サンプルデータや係数の表示/クリアなどのオプションを提供する。
     * @param aMouseEvent マウスイベントオブジェクト
