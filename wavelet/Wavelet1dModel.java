...
public class Wavelet1dModel extends WaveletModel {
        ...
        public static double[] dataSampleCoefficients() {
                ...
                IntStream.range(32, 48)
                                .forEach(index -> arrayOfDouble[index] = Math.pow((48 - index + 1), 2.0D) / 256.0D
                                                - 0.5D); // 次の16要素を設定
        ...
        public static BufferedImage generateImage(double[] valueCollection) {
                int dataLength = valueCollection.length;
                int imageWidth = (int) Math.round(dataLength * scaleFactor.x);
                int imageHeight = (int) Math.round(rangeValue * scaleFactor.y);
                ...
                IntStream.range(0, dataLength)
                                .forEach(dataIndex -> {
                                        ...
                                        int plotY = (int) Math
                                                        .round((0.0D - dataValue) * scaleFactor.y + imageHeight / 2.0D); // Y座標を計算
                                                                                                                         // (Y軸反転)
        ...
        public void open() {
                ...
                WaveletPaneView waveletPaneViewInteractive = new WaveletPaneView(
                                this.interactiveWaveletCoefficientsPaneModel);
                gridBagConstraints.gridx = 2;
                gridBagConstraints.gridy = 1;
                gridBagConstraints.weightx = 0.25D;
                gridBagConstraints.weighty = 0.5D;
        ...
        public void setSourceData(double[] sourceDataArray) {
                ...
                if (this.interactiveWaveletCoefficientsPaneModel == null)
                        this.interactiveWaveletCoefficientsPaneModel = new WaveletPaneModel(null,
                                        "Interactive Wavelet Coefficients", this);
