...
public abstract class DiscreteWaveletTransformation extends WaveletTransformation 
{
  ...
  {
    initialize(2); // デフォルトで次数N=2のDaubechiesウェーブレットを初期化
  }
   ...
   * <p>サポートされる次数は以下の通り。
   * <ul>
   * <li>{@code index == 2}: 4タップフィルター係数（デフォルト）</li>
   * <li>{@code index == 3}: 6タップフィルター係数</li>
  ...
  {
    super.initialize(); // 親クラスの初期化メソッドを呼び出し
