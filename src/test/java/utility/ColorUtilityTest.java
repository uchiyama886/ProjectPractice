...
class ColorUtilityTest {
    ...
    private static final double DELTA = 1e-9; // 浮動小数点比較のための許容誤差
    ...
    void testColorFromLuminance() {
        ...
        // そのため、Color.getRGB() と直接比較するとアルファ成分の有無で値が異なる
        // したがって、期待値も ColorUtility.convertRGBtoINT が返す形式に合わせる
    ...
    void testColorFromYUVArray() {
        ...
        // 実際の変換結果の丸め誤差を考慮して、直接 RGB を計算し、その結果の int 値と比較
        // convertYUVtoRGB(0.587, -0.331, -0.419) -> R:0.003, G:1.000, B:0.001
        int expectedR = (int)Math.round(0.003 * 255.0); // ~0
        ...
        assertEquals(expectedRGBGreen, color.getRGB() & 0xFFFFFF, "Color from YUV array (Green) should be correct within tolerance (alpha ignored)");
    }
    ...
    void testColorFromYUVIndividual() {
        ...
        // convertYUVtoRGB(0.299, -0.169, 0.500) -> R:0.999, G:0.000, B:0.001
