...
class WaveletDataTest {
    ...
    private PrintStream originalErr; // System.err もキャプチャするために追加
    private ByteArrayOutputStream errContent; // System.err もキャプチャするために追加
    ...
    void setUp() {
        ...
        originalErr = System.err; // 追加
        errContent = new ByteArrayOutputStream(); // 追加
        System.setErr(new PrintStream(errContent)); // 追加
    }

    @AfterEach
    void restoreStreams() {
        System.setOut(originalOut);
        System.setErr(originalErr); // 追加
    }
    ...
    void testGenerateImageColor() {
        // 高さ2、幅1の画像を想定して行列を定義します
        // yMatrixDummy の次元も合わせます
        double[][] yMatrixDummy = {{0.0}, {0.0}}; // 高さ2, 幅1
        ...
        // 画像の幅は rMatrix[0].length (1)、高さは rMatrix.length (2) になるようにアサーションを修正
        // image.getWidth() は行列の列数 (rMatrix[0].length) に対応し、
        // image.getHeight() は行列の行数 (rMatrix.length) に対応すると仮定
        assertEquals(rMatrix[0].length, image.getWidth(), "画像の幅は行列の列数と一致するはずです"); // 期待値: 1
        assertEquals(rMatrix.length, image.getHeight(), "画像の高さは行列の行数と一致するはずです"); // 期待値: 2

        // ピクセル (0,0) (x=0, y=0) -> rMatrix[0][0]=0.0, gMatrix[0][0]=1.0, bMatrix[0][0]=0.0 -> Color(0,255,0) (緑)
        assertEquals(new Color(0, 255, 0).getRGB(), image.getRGB(0, 0), "ピクセル (0,0) は緑であるべきです");
        // ピクセル (0,1) (x=0, y=1) -> rMatrix[1][0]=1.0, gMatrix[1][0]=0.0, bMatrix[1][0]=0.0 -> Color(255,0,0) (赤)
        assertEquals(new Color(255, 0, 0).getRGB(), image.getRGB(0, 1), "ピクセル (0,1) は赤であるべきです");
    }
    ...
    void testGenerateImageSingleMatrixRed() {
        ...
        assertNotNull(image);
        // MaxValue は 0.5 (valueMatrix[0][0])
